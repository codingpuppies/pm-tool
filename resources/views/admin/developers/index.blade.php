...
@section('page-header')
    Developers <small>{{ trans('app.manage') }}</small>
@endsection

@section('content')

    <div class="mB-20">
        <a href="{{ route(ADMIN . '.developers.create') }}" class="btn btn-info">
            ...
            <tbody>
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td>{{ $item->email }}</td>
                        <td>{{ config('variables.position')[$item->position] }}</td>
                        <td>{{ config('variables.position')[$item->department] }}</td>
                        <td>
